...
const run = async () => {
  ...
  const g1 = await Guide.findById('69bd56528d5ee4d6a918ab43').populate('ciudad','nombre slug');
  const g2 = await Guide.findById('69bae5e38f6a5296849b2522').populate('ciudad','nombre slug');
  ...
  const all = await Guide.find({}, 'titulo imagen ciudad').populate('ciudad','nombre slug');
  const imgCount = {};
  for (const g of all) {
